...
from .models import Document, Annotation, Comment
from .serializers import DocumentSerializer, AnnotationSerializer, CommentSerializer
...
def _storage_public_or_signed_url(
    supabase: Client, bucket: str, path: str, ttl_seconds: int = 7 * 24 * 3600
) -> str | None:
...
def _upload_drawing_json(
    supabase: Client, bucket: str, path: str, data: dict
) -> str | None:
    try:
        b = json.dumps(
    ...
    def from_gemini(self, request):
        ...
        try:
            ...
            file_path = default_storage.save(
                f"uploads/{f.name}", ContentFile(pdf_bytes)
            )
            ...
            if supabase:
                ...
                _storage_upload_bytes(
                    ...
                    pdf_path,
                    pdf_bytes,
                    _safe_ct(f.name, "application/pdf"),
                )
                storage_pdf_url = _storage_public_or_signed_url(
                    supabase, BUCKET, pdf_path
                )
                ...
                storage_json_url = _storage_public_or_signed_url(
                    supabase, BUCKET, json_path
                )
...
class AnnotationViewSet(viewsets.ModelViewSet):
    queryset = (
        Annotation.objects.select_related("document", "patient")
        .all()
        .order_by("-created_at")
    )
...
class CommentViewSet(viewsets.ModelViewSet):
    queryset = (
        Comment.objects.select_related("document", "patient")
        .all()
        .order_by("-created_at")
    )
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if supabase:
                ...
                ts = int(time.time())
                path = f"{document_id}/{patient_id}/{annotation.id}-{ts}.json"
                storage_url = _upload_drawing_json(
                    supabase, bucket, path, body
                )
...
def build_annotated_pdf(request, document_id):
    ...
    if not annotated:
        return HttpResponseBadRequest(
            "Tidak ada annotated_pages di meta. Simpan halaman terlebih dahulu."
        )
    ...
    if not items:
        return HttpResponseBadRequest(
            "annotated_pages kosong atau tidak valid."
        )
    ...
    for page_num, url in items:
        try:
            ...
            return HttpResponseBadRequest(
                f"Gagal mengambil gambar untuk halaman {page_num}: {e}"
            )
